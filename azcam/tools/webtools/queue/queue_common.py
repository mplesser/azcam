...
class QueueCommon(object):
    """
    The QueueCommon class which implements queue observing scripts.
    """
    ...
    def _get_focus(
        ...
        if self.focus_component == "instrument":
            return azcam.db.instrument.get_focus(focus_id)
        elif self.focus_component == "telescope":
            return azcam.db.telescope.get_focus(focus_id)
    ...
    def _set_focus(
        ...
        if self.focus_component == "instrument":
            return azcam.db.instrument.set_focus(focus_value, focus_id, focus_type)
        elif self.focus_component == "telescope":
            return azcam.db.telescope.set_focus(focus_value, focus_id, focus_type)
    ...
    def run(self, number_cycles=1):
        ...
        for loop in range(number_cycles):
            ...
            if number_cycles > 1:
                self.log("*** Script cycle %d of %d ***" % (loop + 1, number_cycles))
    ...
    def execute_command(self, linenumber):
        ...
        # display message and then change command, for now
            ...
            try:
                reply = server.command(f"telescope.offset {ra} {dec}")
                return "OK"
        ...
        if movefilter_flag:
            if wave != self.current_filter:
                ...
                if not self.mock:
                    azcam.db.instrument.set_filter(wave)
                    reply = azcam.db.instrument.get_filter()
        ...
        if movetel_flag:
            ...
            if not self.mock:
                try:
                    reply = server.command(f"telescope.move {ra} {dec} {epoch}")
                except azcam.AzcamError as e:
                    return f"ERROR {e}"
        ...
        if expose_flag:
            for i in range(numexposures):
                ...
                filename = azcam.db.exposure.get_filename()
                ...
                if self.move_telescope_during_readout and (raNext != ""):
                    ...
                    if 1:
                        # if not self.debug:
                        reply = azcam.db.exposure.expose1(
                            exptime, imagetype, title
                        )  # immediate return
                        time.sleep(2)  # wait for Expose process to start
                        cycle = 1
                        while 1:
                            flag = azcam.db.params.get_par("ExposureFlag")
                            if flag is None:
                    ...
                    if not self.mock:
                        azcam.db.exposure.expose(exptime, imagetype, title)
