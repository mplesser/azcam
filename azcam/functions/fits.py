...
def get_all_data(filename: str = "test") -> List:
    ...
    with pyfits.open(filename) as im:
        ...
        if NumExt == 0:
            ...
            datalist = [im[chan].data for chan in range(first_ext, last_ext)]
    return datalist


def resample(filename: str = "test", resample: int = 2) -> None:
    """
...
def _get_data_roi(azcam_roi=[]):
    ...
    roi = azcam.db.imageroi if azcam_roi == [] else azcam_roi
