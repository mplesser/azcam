...
class FocalPlane(object):
    ...
    def define_keywords(self):
        """
        Defines focal plane keywords, if they are not already defined.
        """

        if len(self.header.keywords) != 0:
            return

        for key in self.header.keywords:  # was keywords
    ...
    def roi_reset(self):
        ...
        self.set_roi(1, reply[1], 1, reply[5], -1, -1)
...
class WCS(object):
    ...
    def get_ra_dec(self):
        ...
        try:
            get_ra = azcam.db.objects[azcam.db.coord_object].header.get_keyword("RA")
        ...
        try:
            get_dec = azcam.db.objects[azcam.db.coord_object].header.get_keyword(
                "DEC"
            )
    ...
    def _dec_to_deg(self, dec):
        ...
        degr = (int(dms[0]) + int(dms[1]) / 60.0 + float(dms[2]) / 3600.0) * sgn
    ...
    def _ra_to_deg(self, ra):
        ...
        degr = (int(hms[0]) + int(hms[1]) / 60.0 + float(hms[2]) / 3600.0) * 15
